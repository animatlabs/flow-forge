...
- **Dependency-Free Core**: The core `WorkflowEngine` project has no external dependencies, making it lightweight and suitable for environments with strict dependency policies.
...
Clone the repository and include the `WorkflowEngine` project in your solution. Since the core library has no dependencies, you can use it directly without worrying about external packages.

Alternatively, you can include additional projects like `WorkflowEngine.Logger.Serilog` or `WorkflowEngine.Middlewares.OpenTelemetry` based on your requirements.
...
var context = new WorkflowContext(
    ...
    workflowStepCompensationManager: new WorkflowStepCompensationManager(),
    workflow: workflow
);
...
For distributed tracing, include the `WorkflowEngine.Middlewares.OpenTelemetry` project and use the `TracingMiddleware`.
...
For logging, include the `WorkflowEngine.Logger.Serilog` project and use the `SerilogLogger`.
